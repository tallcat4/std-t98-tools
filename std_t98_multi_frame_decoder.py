...
def decode_pich(pich_bits_str: str) -> dict:
    ...
    同期バースト(F=0)時のTCH1領域(144bits)がこれに該当する。
...
def get_output_filenames(ch):
    return f"output_ch{ch}.ambe", f"output_ch{ch}.burst"
...
def main():
    ...
    try:
        ...
        while True:
            ...
            print(f"\n====================")
            print(f"[Channel {ch}] Average dt: {avg_dt:.2f} ms | Total Symbols: {symbols.size}")
            ...
            if rich_data is not None:
                # ====== 同期バースト (F=0) の処理 ======
                if rich_data['F'] == 0:
                    print(f"    --> [Channel {ch}][Mode: Sync Burst] Processing TCH1 as PICH...")
                    try:
                    ...
                    print(f"    --> [Channel {ch}] [Mode: Sync Burst] TCH2 is discarded (Not Used).")

                # ====== 通信用バースト (F=1) の処理 ======
                elif rich_data['F'] == 1:
                    print(f"    --> [Channel {ch}][Mode: Traffic Burst] Processing TCH as Voice Data...")
                    
                    if ch not in initialized_channels:
                        ambe_file_path, burst_file_path = get_output_filenames(ch)
                        open(ambe_file_path, "wb").close()
                        open(burst_file_path, "wb").close()
                        initialized_channels.add(ch)
                    ...
                    # バイナリファイルに追記出力
                    with open(ambe_file_path, "ab") as f_ambe:
                        for block in blocks:
                    ...
                    # Burst ファイルに追記出力
                    with open(burst_file_path, "ab") as f_burst:
                    ...
                    print(f"    --> Appended 4 AMBE frames (10 bytes each) to '{ambe_file_path}'")
                    print(f"    --> Appended burst data to '{burst_file_path}'")
