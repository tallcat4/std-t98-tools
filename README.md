...
4. 抽出された `output.ambe`もしくは`output.burst`をThumvDVやmbelibなどでWAV音声などに変換することも可能です。

`output.ambe`は、単純にAMBE+2音声データのみを順番にバイナリに書き出したもの、`output.burst`はこれに加えて、受信したデータにおけるフレームの開始位置を示すデータを加えたものです。
STD-T98では、一フレームに4つのAMBE+2音声データが含まれています。特に秘話処理はフレーム単位で行われるため、どこがフレームの開始位置なのかを把握する必要があります。
秘話の解読処理を行う場合、`output.ambe`ではなく`output.burst`を用いて、フレームの開始位置を示す`0xFF`から４つの音声データを順番に取得する必要があります。

grcファイルにはチャネル1番を復調するためのパラメータが設定済みです。必要に応じてGNURadio Companionでパラメータを変更して使用してください。