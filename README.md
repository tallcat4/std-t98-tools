...
サービスごとの集計値とチャンネル別の debug metrics を dashboard に出したい場合:

```bash
./env/bin/python std_t98_multi_service_launcher.py --show-debug-metrics
```

深掘りする時は `--show-debug-metrics --passthrough-output` を併用すると、launcher の集計と各 child process のログを同時に見られます。

音声再生は既定で PortAudio / PipeWire のデフォルト遅延を使います。低遅延に戻したい場合だけ、起動前に `STD_T98_AUDIO_LATENCY=low` を設定してください。低速 CPU では既定値のほうが安定します。

audio service はチャネルごとの PCM を短時間バッファしてからミックスします。同時発話時の聞き取りやすさを優先して、起動直後の再生には約 120ms のプリバッファを入れています。

再生音量は既定で 3.0 倍の出力ゲインをかけます。環境に応じて `STD_T98_AUDIO_GAIN=2.0` や `STD_T98_AUDIO_GAIN=4.0` のように調整できます。
